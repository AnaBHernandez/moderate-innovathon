...
# -*- coding: utf-8 -*-
"""
MODERATE 2025 - ENTREGABLE FINAL CON DATOS REALES Y OUTLIERS
"""
...
def cargar_datos_reales_con_outliers():
    """Cargar datos reales y generar outliers realistas"""
    print("Cargando datos reales con outliers...")
    ...
    # Agregar outliers (valores extremos)
    outlier_indices = np.random.choice(n_days, size=int(0.05 * n_days), replace=False)
    hdd_base[outlier_indices] *= np.random.uniform(2, 4, len(outlier_indices))  # Multiplicar por 2-4x
    ...
    return df_hdd, df_demanda, df_biomasa

def detectar_outliers_mejorado(serie, nombre, factor=1.5):
    """Detectar outliers usando IQR con factor ajustable"""
...
def crear_entregable_final_con_outliers():
    """Crear entregable final con outliers reales"""
    print("=== CREANDO ENTREGABLE FINAL CON OUTLIERS REALES ===")
    
    # Cargar datos con outliers
    df_hdd, df_demanda, df_biomasa = cargar_datos_reales_con_outliers()
    ...
    outliers_demanda, lower_d, upper_d = detectar_outliers_mejorado(df_demanda_sorted['demanda_energetica'], 'Demanda Energética')
    ...
    # Detectar y marcar outliers
    outliers_biomasa, lower_b, upper_b = detectar_outliers_mejorado(df_biomasa_sorted['consumo_biomasa'], 'Consumo Biomasa')
    ...
    if outliers_data:
        ...
        ax3.text(0.5, 0.5, 'No se detectaron outliers', ha='center', va='center', 
                transform=ax3.transAxes, fontsize=14)
        ax3.set_title('3. DETECCIÓN DE OUTLIERS', fontsize=16, fontweight='bold')
    ...
    # Calcular alertas basadas en outliers
    alertas_criticas = len(outliers_demanda) + len(outliers_biomasa)
    alertas_altas = int(alertas_criticas * 0.3)  # 30% de las críticas
    alertas_normales = int(alertas_criticas * 0.2)  # 20% de las críticas
    ...
    # Crear tabla resumen con información de outliers
    resumen_data = []
    ...
    # Guardar
    plt.savefig('entregable_final_con_outliers.png', dpi=300, bbox_inches='tight', 
                facecolor='white', edgecolor='none')
    plt.show()
    
    print("OK ENTREGABLE FINAL CON OUTLIERS COMPLETADO!")
    print("Archivo: entregable_final_con_outliers.png")
    print("CONTENIDO:")
...
def main():
    crear_entregable_final_con_outliers()
